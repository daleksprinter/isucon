...
var mySQLConnectionEstateData *MySQLConnectionEnv
...
func main() {
	...
	chairDB, err = mySQLConnectionChairData.ConnectDB()
	if err != nil {
...
func postEstate(c echo.Context) error {
	...
	for _, row := range records {
		...
		values = fmt.Sprintf("%s(%d,%s,%s,%s,%s,%f,%f,%d,%d,%d,%s,%d),", values, id, name, description, thumbnail, address, latitude, longitude, rent, doorHeight, doorWidth, features, popularity)
	}
	values = values[:len(values)-1]
...
func getMax(a, b, c int64) (mx1, mx2 int64) {
	arr := []int64{a, b, c}
...
func searchRecommendedEstateWithChair(c echo.Context) error {
	...
	err = estateDB.Select(&estates, query, mx1, mx2, mx2, mx1, Limit)
